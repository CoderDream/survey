...
public class SelecterService {
	...
	public List listSelecterBySeq(int seq, int oid) {
		...
		String sql = "select qseq,selseq,content from wj_selecter where qseq = '" + seq + "' and oid = '" + oid
				+ "' order by selseq asc";
	...
	public int addSelecter(int oid, int seq, String content, int seq_selecter) {
		...
		Statement stm = null;
		ResultSet rs = null;
		String sql = "insert into wj_selecter(oid,qseq,content,selseq) values('" + oid + "','" + seq + "','" + content
				+ "','" + seq_selecter + "')";
		System.out.println(sql);
		try {
			dbcon = new DBConnection();
	...
	public int updateSelecterSeq(int oid, int qseq) {
		...
		Statement stm = null;
		ResultSet rs = null;
		String sql = "update wj_selecter set qseq=(qseq+1) where oid = '" + oid + "'and qseq > '" + qseq + "'";
		System.out.println(sql);
		try {
			dbcon = new DBConnection();
			con = dbcon.getConnection();
	...
	public int updateSseq(int seq, int oid) {
		...
		ResultSet rs = null;
		int count = 0;
		String sql = "update wj_selecter set qseq=(qseq-1) where oid = " + oid + " and qseq > " + seq;
