...
public class QuestionService {
	...
	public List<Question> litQuesByOid(int oid) {
		...
		String sql = "select seq,content,qtype from wj_question where oid='" + oid + "' order by seq asc";
		try {
	...
	public int addQues(int oid, String content, int qtype, int seq) {
		...
		Statement stm = null;
		ResultSet rs = null;
		String sql = "insert into wj_question(oid,content,qtype,seq) values('" + oid + "','" + content + "','" + qtype
				+ "','" + seq + "')";
		try {
			dbcon = new DBConnection();
			con = dbcon.getConnection();
	...
	public Question getQuesBySeq(int seq, int oid) {
		...
		ResultSet rs = null;
		Question ques = new Question();
		String sql = "select content,qtype from wj_question where oid = '" + oid + "' and seq = '" + seq + "'";
		try {
			dbcon = new DBConnection();
			con = dbcon.getConnection();
			stm = con.createStatement();
